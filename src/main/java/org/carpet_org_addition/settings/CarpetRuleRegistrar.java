...
package org.carpet_org_addition.settings;
            ...
            if (rule != null) {
                for (Class<? extends RuleObserver> observer : rule.observer()) {
                    ...
                    try {
                        ...
                        CarpetOrgAddition.LOGGER.warn("Failed to invoke observer {} for rule {}: {}", observer, field.getName(), e);
                    }
    ...
    private void registerToCarpet() {
        ...
        for (CarpetRule<?> rule : this.rules) {
            try {
                ...
                CarpetOrgAddition.LOGGER.warn("Failed to register rule {} to fabric carpet: {}", rule.name(), e);
            }
        }
        //#else
